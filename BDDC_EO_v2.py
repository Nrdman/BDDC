...
def Fc(x):
   ...
   # global lii
    ...
    for i in range(DomNum):
        ...
        temp2 = np.zeros([n,n])
        for j in range(n):
            temp2[j,:] = domain_K[i][lb[j], lb].toarray()
            #removed a loop here
        temp3 = np.linalg.solve(Ri.T, temp_matrix.T@w[lb])
        temp3 = np.linalg.solve(Ri, temp3)
        yy[lb, :] = temp2@w[lb, :]-temp_matrix@temp3
...
def pc(x):
    ...
    for i in range(DomNum):
        for j in range(lb.size):
            for k in range(lb.size):
                D_lb[i, j,k] = domain_D[i][lb[j],lb[k]]
            # removed a loop here 
        for j in range(lcc.size):
            K_cr[i,j,:] = domain_K[i][lcc[j],lrr].toarray()
            #removed a loop here
        K_rc[i,:,:] = K_cr[i,:,:].T
    for i in range(DomNum):
        lx = np.zeros([lnv,1])
        ...
        lx[lb] = D_lb[i,:,:]@(domain_B[i][lb,:]@x) 
        ytemp=np.linalg.solve(domain_KrrR[i,:,:].T, lx[lrr])
        ytemp = np.linalg.solve(domain_KrrR[i, :, :], ytemp)
        llx[lrr] = ytemp
        
        y1 = y1+domain_B[i][lb,:].T@(D_lb[i,:,:].T@llx[lb])
        lx[lcc]=lx[lcc]-K_cr[i,:,:]@ytemp
    ...
    for i in range(DomNum):
        ...
        ytemp = np.linalg.solve(domain_KrrR[i,:,:], ytemp)
        lx[lrr] = -ytemp
        y3 = y3+domain_B[i][lb,:].T@(D_lb[i,:,:].T@lx[lb])
    y=y1+y3
    return y
...
def BDDC(Nvm, nvm, a1, a2):
    ...
    for i in range(dsize):
        domain_K.append(sparse.csr_matrix((eachnv, eachnv), dtype=np.float32))
        domain_B.append(sparse.csr_matrix((lnv, ng), dtype=np.float32))
        #domain_B.append(np.zeros([lnv, ng]))
        domain_Bc.append(sparse.csc_matrix((lnv, nc), dtype=np.float32))
    ...
    for ii in range(Nvm):
        for jj in range(Nvn):
            ...
            domain_KiiR[dindex,:,:] = scipy.linalg.cholesky(temp_matrix);
            
            temp_matrix = np.zeros([lrr.size, lrr.size]);
            for m in range(lrr.size):
                temp_matrix[m, :] = aK[lrr[m], lrr[:]].toarray();
                #loop removed here
            domain_KrrR[dindex, :, :] = scipy.linalg.cholesky(temp_matrix);
    ...
    for i in range(DomNum):
        ...
        for lbindex in range(max(lb.shape)):
            ...
            #removed loop here
        temp_matrix = domain_Kbi[i, :, :]
        update = ((domain_B[i][lb,:]).T) @ (domain_fb[i, :]-temp_matrix@lfi)
        ...
        KccS = KccS+domain_Bc[i][lcc, :].T@((domain_Kcc[i]-domain_Krc[i, :, :].T@temp_matrix)@domain_Bc[i][lcc, :]);
    ...
    for i in range(DomNum):
        ...
        domain_u[i, lii]=np.linalg.solve(domain_KiiR[i, :, :], ytemp) 
    resl2 = 0
    errmax = 0
    for i in range(DomNum):
